    ...
    <main className="min-h-screen bg-ink text-porcelain">
      ...
      <section id="heritage-list" className="mx-auto max-w-7xl px-5 py-10">
        <div className="mb-8 flex flex-wrap gap-2">
          <Link className={`rounded-full border px-4 py-2 text-sm ${!selected ? "border-gold bg-gold text-ink" : "border-white/15 bg-white/[0.04] text-linen/75"}`} href="/heritage">
            Барлығы
          </Link>
          {categories.map((category) => (
            <Link className={`rounded-full border px-4 py-2 text-sm ${selected === category ? "border-gold bg-gold text-ink" : "border-white/15 bg-white/[0.04] text-linen/75"}`} href={`/heritage?category=${encodeURIComponent(category)}`} key={category}>
              {category}
            </Link>
          ))}
        </div>
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => (
            <Link className="group overflow-hidden rounded-md border border-white/10 bg-white/[0.04]" href={`/heritage/${item.slug}`} key={item.slug}>
              <Image src={item.image} alt={item.title.kk} width={720} height={460} className="h-52 w-full object-cover transition duration-500 group-hover:scale-105" />
              <div className="p-5">
